CLASS ypayment_advice_for_mail DEFINITION
 PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS YPAYMENT_ADVICE_FOR_MAIL IMPLEMENTATION.
...
*    AND accountingdocument = @document  and CompanyCode = @comcode INTO TABLE @DATA(tab1).

    SELECT  * FROM i_operationalacctgdocitem WHERE (where)
    AND ClearingJournalEntryFiscalYear = @todate AND  AccountingDocumentType <> 'KZ'
    AND ClearingJournalEntry = @document  and CompanyCode = @comcode INTO TABLE  @DATA(tab1).

*   FOR PARTIAL PAYMENT CASE ON DEMAND ON DEV SHARMA ON 11.06.2025
    IF tab1 IS INITIAL.

    SELECT  * FROM i_operationalacctgdocitem WHERE (where)
    AND accountingdocumenttype = 'KZ'
    AND accountingdocument = @document and FiscalYear = @fromdate and CompanyCode = @comcode INTO TABLE @tab1.
    ...
    AND ClearingJournalEntry = @document  and CompanyCode = @comcode INTO TABLE  @tab1.
...
*    ELSE .
    ...
    AND CompanyCode = @wa_tab1-CompanyCode INTO @DATA(journalentry).
...
IF PLANTADD-CompanyCode ='1000'.
  DATA(Register)  =  'Natural Resources Limited                         '  .
...
and a~enq_status = 'Processed'
and c~emailaddress <> ''
into table @data(utrno).


 LOOP AT utrno INTO DATA(wa1).
     clear :  to_mail.
